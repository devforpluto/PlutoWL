...
const { removeBlacklist } = require("../../utils/luawl");
...
create_command(
      ...
      await interaction.followUp({
        embeds: [
          success_embed.setDescription(
            Formatters.bold(
              `Successfully removed the blacklisted for ${member.user.toString()}`
            )
          ),
        ],
      });
      ...
      member.user
        .send({
          embeds: [
            success_embed
              .setDescription(
                Formatters.bold("Your blacklist has been removed")
              )
              .setAuthor({
  ...
  },
  {
    ...
    description: "Removes the blacklist from the mentioned user",
